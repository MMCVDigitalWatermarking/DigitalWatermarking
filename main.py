...
class Ui_MainWindow(object):
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.DCT_masg.setText(QCoreApplication.translate("MainWindow", u"num_of_co=1000, alpha=0.1", None))
    ...
    def method_LSB_decode(self, progress_callback):
        self.disable_Methods(True)
        self.lsb_encode_msg.setText("Decoded msg:")
    ...
    def call_lsb_decode(self):
        self.show_progress_bar()
        # Pass the function to execute
        worker = Worker(self.method_LSB_decode)  # Any other args, kwargs are passed to the run function
        worker.signals.result.connect(self.print_output)
        worker.signals.finished.connect(self.thread_complete)
        worker.signals.progress.connect(self.progress_fn)
